...
function renderHtml(opts: ComposePromoOptions): string {
  ...
  const ctaButton = ctaUrl
    ? `<a href="${escapeHtml(ctaUrl)}" style="display:inline-block;background:${COLORS.warmWhite};color:${COLORS.black};padding:16px 40px;font-family:'Outfit',-apple-system,sans-serif;font-weight:700;font-size:15px;letter-spacing:0.4px;text-decoration:none;">${escapeHtml(ctaLabel)}</a>`
    : ''
  ...
  <tr><td align="center" style="padding:48px 16px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" class="wrap" style="width:600px;max-width:600px;background:${COLORS.warmWhite};">
      ...
      <tr><td style="padding:40px 0 8px 0;">
        <h1 class="promo-headline" style="margin:0 0 20px 0;font-family:'Outfit',-apple-system,sans-serif;font-weight:800;font-size:34px;line-height:40px;letter-spacing:-0.8px;color:${COLORS.black};">
          ${escapeHtml(headline)}
        </h1>
        ${bodyHtml}
      </td></tr>
